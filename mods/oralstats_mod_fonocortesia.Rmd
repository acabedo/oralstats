---
title: "Oralstats. Fonocortesía mod"
author: "Adrián Cabedo Nebot / Antonio Hidalgo Navarro"
...
```{r global,cache=TRUE,include=FALSE,echo=FALSE}
...
ip <- read.fst("fonocortesia2.fst")
ip <- as_data_frame(ip)
# ip <- readRDS("fonocortesia.RDS")
# ip <- fst::read_fst("fonocortesia.fst")
...
Data proudly visualized with Oralstats. *Versión beta: 1.0* (expect any
kind of bugs). Free available at <https://github.com/acabedo/oralstats>

# Info {.tabset .tabset-fade}

This dynamic dashboard has been generated with
[Oralstats](https://github.com/acabedo/oralstats). It includes 278
intonational phrases from Fonocortesia (Phonopoliteness), a Spanish
research project ended in 2013.
...
intonational phrases, the most common speech unit.

## Authors

+-------------------------------------+--------------------------------+
| Fonocortesia Project Manager        | Project collaborator and tool  |
|                                     | creator                        |
+:===================================:+:==============================:+
| ![](hidalgo.jpeg){width="200"}      | ![](cabedo2.jpeg){width="157"} |
+-------------------------------------+--------------------------------+
| Antonio Hidalgo Navarro\            | Adrián Cabedo Nebot\           |
| ([antonio.hidalgo\@uv               | ([adrian.cabedo\@uv.es         |
| .es](mailto:antonio.hidalgo@uv.es)) | ](mailto:adrian.cabedo@uv.es)) |
+-------------------------------------+--------------------------------+
...
```{r inputpanel, include=TRUE,cache=TRUE, echo=FALSE, message=FALSE,warning=FALSE}
...
tags$table(
  ...
  tags$tr(tags$th(
pickerInput("units","Select units", choices = "ip" , options =  list(`actions-box` = FALSE),multiple = F, selected = "ip", width = "250px"))),
...
```{r filtered, cache=TRUE,include=TRUE,echo=FALSE, message=FALSE,warning=FALSE}
...
Sys.sleep(5)
  ...
  filtered <- p %>% mutate(f1 = !!input$vars1, f2 = !!input$vars2, f3 = !!input$vars3, f4 = !!input$vars4, f5 = !!input$vars5, f6 = !!input$vars6)
   filtered <- if
    ...
    {filtered} else
      if
    (input$filter6 == ""  & !is.null(input$filter1) & is.null(input$filter2) & is.null(input$filter3) & !is.null(input$filter4) & !is.null(input$filter5))
      {filtered%>%filter(f1 %in% !!input$filter1,f4 >= !!input$filter4[1], f4 <= !!input$filter4[2],f5 >= !!input$filter5[1], f5 <= !!input$filter5[2])}
   else
      if
    (input$filter6 == ""  & is.null(input$filter1) & is.null(input$filter2) & is.null(input$filter3) & !is.null(input$filter4) & !is.null(input$filter5))
    {filtered%>%filter(f4 >= !!input$filter4[1], f4 <= !!input$filter4[2],f5 >= !!input$filter5[1], f5 <= !!input$filter5[2])}
          ...
          if
    (input$filter6 == ""  & !is.null(input$filter1) & !is.null(input$filter2) & is.null(input$filter3) & !is.null(input$filter4) & !is.null(input$filter5))
    {filtered%>%filter(f1 %in% !!input$filter1,f2 %in% !!input$filter2,f4 >= !!input$filter4[1], f4 <= !!input$filter4[2],f5 >= !!input$filter5[1], f5 <= !!input$filter5[2])}
        else
                 if
    (input$filter6 == ""  & !is.null(input$filter1) & !is.null(input$filter2) & !is.null(input$filter3) & !is.null(input$filter4) & !is.null(input$filter5))
    {filtered%>%filter(f1 %in% !!input$filter1,f2 %in% !!input$filter2,f3 %in% !!input$filter3,f4 >= !!input$filter4[1], f4 <= !!input$filter4[2],f5 >= !!input$filter5[1], f5 <= !!input$filter5[2])}
        else
          if
    (input$filter6 != ""  & !is.null(input$filter1) & !is.null(input$filter2) & !is.null(input$filter3) & !is.null(input$filter4) & !is.null(input$filter5))
    {filtered%>%filter(grepl(!!input$filter6,f6),f1 %in% !!input$filter1,f2 %in% !!input$filter2,f3 %in% !!input$filter3,f4 >= !!input$filter4[1], f4 <= !!input$filter4[2],f5 >= !!input$filter5[1], f5 <= !!input$filter5[2])}
        else
        if
    (input$filter6 != ""  & !is.null(input$filter1) & !is.null(input$filter2) & is.null(input$filter3) & !is.null(input$filter4) & !is.null(input$filter5))
    {filtered%>%filter(grepl(!!input$filter6,f6),f1 %in% !!input$filter1,f2 %in% !!input$filter2,f4 >= !!input$filter4[1], f4 <= !!input$filter4[2],f5 >= !!input$filter5[1], f5 <= !!input$filter5[2])}
        else
                  if
    (input$filter6 != ""  & !is.null(input$filter1) & is.null(input$filter2) & is.null(input$filter3) & !is.null(input$filter4) & !is.null(input$filter5))
    {filtered%>%filter(grepl(!!input$filter6,f6),f1 %in% !!input$filter1,f4 >= !!input$filter4[1], f4 <= !!input$filter4[2],f5 >= !!input$filter5[1], f5 <= !!input$filter5[2])}
        else
                            if
    (input$filter6 != ""  & is.null(input$filter1) & is.null(input$filter2) & is.null(input$filter3) & !is.null(input$filter4) & !is.null(input$filter5))
 ...
 #                   (input$filter6 == ""  & !is.null(input$filter1) & !is.null(input$filter2)& !is.null(input$filter3) & is.null(input$filter4) & is.null(input$filter5))
 # 
 #    {filtered %>% filter(f1%in%!!input$filter1, f2 %in% !!input$filter2, f3 %in% !!input$filter3 )}
 #          else
 #                                                 if
 #                   (input$filter6 == ""  & is.null(input$filter1) & !is.null(input$filter2)& is.null(input$filter3) & is.null(input$filter4) & is.null(input$filter5))
 # 
 ...
 #                   (input$filter6 == ""  & !is.null(input$filter1) & !is.null(input$filter2)& !is.null(input$filter3) & !is.null(input$filter4) & !is.null(input$filter5))
 # 
 #    {filtered %>% filter(f4 >= !!input$filter4[1], f4 <= !!input$filter4[2],f5 >= !!input$filter5[1], f5 <= !!input$filter5[2] ,f3%in%input$filter3 ,f2%in%input$filter2, f1%in%input$filter1 )}
 # 
 #          else
 #                                
 #                                                  if
 #                   (input$filter6 != ""  & !is.null(input$filter1) & !is.null(input$filter2)& !is.null(input$filter3) & !is.null(input$filter4) & !is.null(input$filter5))
 # 
 #    {filtered %>% filter(grepl(!!input$filter6,f6),f4 >= !!input$filter4[1], f4 <= !!input$filter4[2],f5 >= !!input$filter5[1], f5 <= !!input$filter5[2] ,f3%in%input$filter3,f2%in%input$filter2, f1%in%input$filter1 )}
 # 
 #          else
 #                                                            
 #                                                                          if
...
corr <- eventReactive(input$corrbutton, {
  mapa <-  filtered()[,-1] 
  mapa1 <- mapa %>%select(where(is.numeric))%>% drop_na()
corr<- cor(mapa1)})
...
pcatest <- eventReactive(input$pcabutton, {
  ...
  # PCA(filtered()%>% mutate(p = !!input$pca2)%>%filter(p %in% !!input$pca3)%>%select(p,!!!input$pca1)%>%drop_na(),quali.sup = 1)
  prcomp(filtered()%>% mutate(p = !!input$pca2)%>%filter(p %in% !!input$pca3)%>%select(!!!input$pca1)%>%drop_na(), scale. = TRUE)
  # res.pca = PCA(filtered()%>% mutate(p = !!input$pca2)%>%select(p,!!!input$pca1)%>%filter(p %in% !!input$pca3)%>%drop_na(), scale.unit=TRUE, ncp=5, quali.sup=1, graph=T)

})

pcatest2 <- eventReactive(input$pcabutton, {

  p <- filtered()%>% mutate(p = !!input$pca2)%>%filter(p %in% !!input$pca3)%>%select(p,!!!input$pca1)%>%drop_na()
...
renderPlot({
 pi<-pcatest2()
  ...
  # autoplot(pcatest(), data=pi, colour = "p", labels=TRUE)
  
  fviz_pca_ind(pcatest(), habillage=pi$p, palette = "Dark2")
...
renderPrint({
 ...
 res.var$cos2
  ...
  tags$strong("Dimension description")
renderPrint({
...
selected file. You can use *corpus* variable to select all the records
in the database.
